...
;===============================================================================
; "Boot to BASIC" for the Space-Time Productions MCB
; Replaces Monitor Rom IC-21 (4K) and adds IC-22 (4K)
;
; BASIC language interpreter
; (c) 1978 Microsoft
; Originally written for the NASCOM(c) Z80 computer
; Ported over for Space-Time Productions MCB Board 2004-2005
; 
; Adjusted to allow 'Boot to BASIC' Nov 2010-Feb 2011
;===============================================================================
;------------------------------------------------------------------------------
; Memory Map
;------------------------------------------------------------------------------
RAM:	.EQU	$3000		; STACK BOT        ($3000-307F) [128]
STACK:	.EQU	$3080		; STACK TOP        ($3000-307F)
INBUFF:	.EQU	$3080		; INPUT  BUFFER    ($3080-30CF)
BUFFER:	.EQU	$30D0		; CRUNCH BUFFER    ($30D0-311F)
BASICV:	.EQU	$3120		; BASIC VAR SPACE  ($3120-3219)
TEXT:	.EQU	$3220		; BASIC PROGRAM    ($3220-37FF)
;------------------------------------------------------------------------------
;General Equates
;------------------------------------------------------------------------------	
LF:	.EQU	$0A		; LINE FEED
...
;------------------------------------------------------------------------------
WRKSPC:	.EQU	BASICV		; $3120, can be relocated dynamically if needed
...
READFG:	.EQU	BASICV+$54	; READ/INPUT FLAG
LINEAT:	.EQU	BASICV+$55	; Current line number

RAMTOP:	.EQU	BASICV+$60	; Physical end of RAM
PROGST:	.EQU	BASICV+$62	; START OF BASIC TEXT AREA
...
INTVECT:	.EQU	BASICV+$F4	; Soft configurable INT JP command
NMIVECT:	.EQU	BASICV+$F7	; Soft configurable NMI JP command
...
RST10:	JP	RXA
	...
	NOP
;------------------------------------------------------------------------------
; TX a character over RS232 Channel B, wait for TXDONE first.
;------------------------------------------------------------------------------
RST18:	JP	TXB
	NOP
	NOP
	...
	NOP
;------------------------------------------------------------------------------
; RX a character from RS232 Channel B, hold here until char ready.
;------------------------------------------------------------------------------
RST20:	JP	RXB
	NOP
	NOP
...
;------------------------------------------------------------------------------
RST38:	JP	INTVECT		; Software config JP, default WSTART
;------------------------------------------------------------------------------	
; Print a string message terminated by $00
;------------------------------------------------------------------------------	
PRT:	LD	A,(HL)		; Get character
	OR	A		; Test for zero
	RET	Z		; Return if finished
	RST	08H		; Print the character in A
	INC	HL		; Point to next character
	JR	PRT		;  and loop thru until finished
;------------------------------------------------------------------------------	
; CLear Screen by printing a Form Feed character $0C
;------------------------------------------------------------------------------
CLS:	LD	A,$0C		; Form feed
	RST	08H		; Print it
	RET
;------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------
RX8279:	IN	A,($79)		; Get p8279 Status
	AND	$07		; 0-7 keys in buffer?
	JR	Z,RX8279		; No, loop until key
	IN	A,($78)		; Get the key character and ret
	RET	
;------------------------------------------------------------------------------
; $0066 - Non-Maskable Interrupt Vector
;------------------------------------------------------------------------------
NMI:	.ORG	$0066
	JP	NMIVECT		; Soft configurable JP default WSTART
;------------------------------------------------------------------------------
; B A S I C	Cold	Start
;------------------------------------------------------------------------------
;------------------------------------------------------------------------------
BASIC:	.ORG	$0070		; Start after RST's and NMI vectors
CSTART:	LD	SP,STACK		; Set SP pointer
INIT:	LD	HL,INITAB		; Source=Init table
	...
	LDIR			;   Make the move
	LD	HL,INTSRC		; Get default INT and NMI defaults
	LD	DE,INTVECT	; Copy them to alterable ram
	LD	BC,$0006		; 2 JP commands
	LDIR			;   Make the move
;------------------------------------------------------------------------------
; USE ONBOARD MEMORY $3000-$37FF ONLY
; These values were calc'd in version that scanned top and prog start of ram
;------------------------------------------------------------------------------
NORAM:	LD	HL,$3220		; SET UP BOARD TO USE ONLY LOCAL 2K SPACE
	LD	(PROGST),HL	; SAVE IT
	LD	(BASTXT),HL	; SAVE IT FOR START OF PROGRAM TEXT
	LD	HL,$37FF		; SET ROOF AT $37FF FOR LOCAL ONLY RAM
	LD	(RAMTOP),HL	; PHYSICAL TOP OF RAM (WHAT THE HARDWARE HAS)
	LD	(LSTRAM),HL	; LOGICAL	TOP OF RAM (CAN BE CHANGED BY CLEAR)
	LD	HL,$379B		; -100 BYTES FOR STRING SPACE LOCATION
	LD	(STRSPC),HL	; SAVE STRING SPACE (STRSPC=LSTRAM-100)
	LD	HL,$05DD		; Remaining after $379B-(25)-PROGST
	LD	(FRERAM),HL	; SAVE IT
	LD	(SYSRAM),HL	; STORE SYSTEM RAM (SYSRAM=RAMTOP-PROGST-1)
	CALL	CLRPTR		; CLEAR POINTERS AND SET UP PROG AREA
;------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------
	...
	LD	HL,2048		; 4 x 2114 1Kx8 Rams = 2K
...
;------------------------------------------------------------------------------
;------------------------------------------------------------------------------
WSTART:	EI			; Enable INTerrupts to system
...
ENFMEM:	PUSH	HL		; Save code string address
	...
	LD	DE,RAM+4		; Nearing lowest available stack position $3000
	SBC	HL,DE		; Subtract current SP-RAM ($30xx-$3004)
...
SNERR:	LD	E,SN		; ?SyNtax Error
	.BYTE	01H		; Skip "LD E,DZ" using "LD BC,(nnnn)"
...
TMERR:	LD	E,TM		; ?TM Error
	.BYTE	01H		; Skip "LD E,SO"
SOERR:	LD	E,SO		; ?Stack Overflow
	.BYTE	01H		; Skip "LD E,OM"
OMERR:	LD	E,OM		; Error - Out of Memory
	.BYTE	01H		; Skip "LE E,HX"
...
LISTLP:	POP	HL		; Restore address of line
	...
	CALL	CKSIOA		; Ck SIO status
	JR	NC,LIST0		; No key, continue
...
LIST0	PUSH	BC		; Save address of next line
...
UPDATA:	LD	(NXTDAT),HL	; Update DATA pointer
	...
	RET
;------------------------------------------------------------------------------
; Check for BREAK during RUN or LIST, process Scroll Controls <Ctrl-S>,<Ctrl-Q>
;------------------------------------------------------------------------------
TSTBRK:	CALL	CKSIOA		; Ck SIO status
...
; DATA/REM
;------------------------------------------------------------------------------
DATA:	.BYTE	$01,$3A		; ":" End of statement
...
BADINP:	LD	A,(READFG)	; READ or INPUT?
	OR	A
	JP	NZ,DATSNR		; READ - ? Data Syntax Error
...
TSTNUM:	.BYTE	$F6		; Clear carry (numeric)
...
FRE:	LD	HL,(ARREND)	; Start of free memory
	EX	DE,HL		; To DE
	LD	HL,(LSTRAM)	; Top of available (after CLEAR)
...
TESTR:	OR	A		; Test if enough room (string length=A)
	.BYTE	$0E		; No garbage collection done
...
POKE:	CALL	GETNUM		; Get memory address
	...
	RET
;------------------------------------------------------------------------------
; HEX( [Replaces DEEK] Convert 16 bit number to Hexadecimal string
;------------------------------------------------------------------------------
...
UNITY:	.BYTE	$00,$00,$00,$81 ; 1.00000

LOGTAB:	.BYTE	3		; TABLE USED BY LOG
	.BYTE	$AA,$56,$19,$80 ; 0.59898
	.BYTE	$F1,$22,$76,$80 ; 0.96147
	.BYTE	$45,$AA,$38,$82 ; 2.88539
;------------------------------------------------------------------------------
; LOG
;------------------------------------------------------------------------------
...
TSTSGN:	LD	A,(FPEXP)		; Get sign of FPREG
	...
	.BYTE	0FEH		; Test sign = CP next byte
RETREL:	CPL			; Invert sign, compiles as 02FH
...
HALF	.BYTE	$00,$00,$00,$80		;0.5
;------------------------------------------------------------------------------
POWERS	.BYTE	$A0,$86,$01	; 100000
	.BYTE	$10,$27,$00	; 10000
	.BYTE	$E8,$03,$00	; 1000
	.BYTE	$64,$00,$00	; 100
	.BYTE	$0A,$00,$00	; 10
...
EXPTAB:	.BYTE	8		; Table used by EXP
	.BYTE	$40,$2E,$94,$74	; -1/7! (-1/5040)
	.BYTE	$70,$4F,$2E,$77	; 1/6! ( 1/720)
	.BYTE	$6E,$02,$88,$7A	; -1/5! (-1/120)
	.BYTE	$E6,$A0,$2A,$7C	; 1/4! ( 1/24)
	.BYTE	$50,$AA,$AA,$7E	; -1/3! (-1/6)
	.BYTE	$FF,$FF,$7F,$7F	; 1/2! ( 1/2)
	.BYTE	$00,$00,$80,$81	; -1/1! (-1/1)
	.BYTE	$00,$00,$00,$81	; 1/0! ( 1/1)
...
;------------------------------------------------------------------------------
WAITCR:	RST	10H		; Get a character in
	CP	$03		; Is it <Break>?
...
LOAD:	CP	ZTIMES		; "*" token?
	...
	CALL	LINTHX		; Load INTEL hex file	

	EXX			; Reclaim the registers BC,DE,HL
	EX	AF,AF'		; Reclaim registers AF
	JP	WSTART		; Warm start the new program
;------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------
LINTHX:	LD	A,CR	; Transmit CR LF first
	RST	08H
	LD	A,LF
	RST	08H
LOAD0:	CALL	LOADC	; See if user types <ctrl-c>, or <spc>, else RET
...
LOAD1:	CP	$3A	; Colon <:>? Start of line of Intel Hex file
	...
	CALL	LOADC2	; Get us two characters into BC, convert it to a byte <A>
	LD	D,A	; Load Record Length count into D
	CALL	LOADC2	; Get next two characters, Memory Load Address <H>
	LD	H,A	; put value in H register.
	CALL	LOADC2	; Get next two characters, Memory Load Address <L>
	LD	L,A	; put value in L register.
	CALL	LOADC2	; Get next two characters, Record Field Type
	CP	$01	; Record Field Type 00 is Data, 01 is End of File
	JR	NZ,LOAD2	; Must be the end of that file
	CALL	LOADC2	; Get next two characters, assemble into byte
...
LOAD2:	LD	A,D	; Retrieve line character counter	
	...
	CALL	LOADC2	; Get next two chars, convert to byte in A, checksum it
...
LOAD3:	CALL	LOADC2	; Get two chars, build byte and checksum
...
LOADERR:	LD	HL,CKSUMERR	; Get "Checksum Error" message
	CALL	PRS	; Print Message from (HL) and terminate the load
	JP	WSTART	; Restart BASIC warm
;------------------------------------------------------------------------------
; Load a character from the console, 
; must be $20-$7F to be valid (no control characters)
; <Ctrl-c> breaks with the Zero Flag set
;------------------------------------------------------------------------------	
LOADC:	RST  10H		; RX a Character
	CP   $03		; <ctrl-c> User break?
	RET  Z			
	CP   $20		; <space> or better?
	JR   C,LOADC	; Do it again until we get something usable
	RET
;------------------------------------------------------------------------------
; Loads two ASCII characters from the console (assuming them to be HEX 0-9 A-F)
; into B and C, converts them into a byte value in A and updates Checksum in E
;------------------------------------------------------------------------------
LOADC2:	CALL LOADC	; Get us a valid character to work with
	LD   B,A		; Load it in B
	CALL LOADC	; Get us another character
	LD   C,A		; load it in C
...
SAVE:	CP	ZTIMES		; "*" Token following? ("SAVE*")
	...
	EXX			; Save BC,DE,HL
	LD	DE,$3120		; Start of BASIC vars storage	
	LD	HL,(ARREND)	; End of Arrays
...
;------------------------------------------------------------------------------
; SAVE DE thru HL in Intel Hex format
;------------------------------------------------------------------------------
...
LSTREC:	LD	A,E		; BETWEEN $00 AND $FF BYTES LEFT TO SAVE
	...
	RST	08H		; PRINT THE CR
	LD	A,$0A		; LOAD A LF
	RST	08H		; PRINT IT
	LD	A,':'		; EOF LINE
	RST	08H		; PRINT THE COLON AT START OF LINE
...
HEXOP:  	AND 	$0F		;Convert the nybble at D3-D2-D1-D0 to Hex ASCII char
	...
	RET
;------------------------------------------------------------------------------
; The EXIT command used to exit BASIC to monitor, now it just resets BASIC
;------------------------------------------------------------------------------
EXIT:	JP	$0000		; No monitor to Exit to...
;------------------------------------------------------------------------------
;------------------------------------------------------------------------------
...
WORDS:	.BYTE	'E'+ $80,"ND"
	...
	.BYTE	'C'+80H,"LS"
	.BYTE	'W'+80H,"IDTH"
	.BYTE	'E'+80H,"XIT"		; formerly MONITR
	.BYTE	'S'+80H,"ET"
	.BYTE	'R'+80H,"ESET"
	.BYTE	'P'+80H,"RINT"
	.BYTE	'C'+80H,"ONT"
...
;------------------------------------------------------------------------------
INITAB:	JP	WSTART		; Warm start jump, located at BASICV	$00-$02
	...
	.BYTE	$00,$00,$00		; Random number seed $17-19

		; Table used by RND
	.BYTE	$35,$4A,$CA,$99		; -2.65145E+07 $1A-3D
	.BYTE	$39,$1C,$76,$98		;  1.61291E+07
	.BYTE	$22,$95,$B3,$98		; -1.17691E+07
	.BYTE	$0A,$DD,$47,$98		;  1.30983E+07
	.BYTE	$53,$D1,$99,$99		; -2-01612E+07
	.BYTE	$0A,$1A,$9F,$98		; -1.04269E+07
	.BYTE	$65,$BC,$CD,$98		; -1.34831E+07
	.BYTE	$D6,$77,$3E,$98		;  1.24825E+07
	.BYTE	$52,$C7,$4F,$80		; Last random number

	IN	A,(0)		; INP (x) skeleton $3E-3F
	RET			; $40

	.BYTE	$01		; Number of NULLs		$41
	.BYTE	79		; Terminal width (79)	$42
	.BYTE	5		; Width for commas (16 columns)$43
	.BYTE	$00		; Null after input byte flag	$44
	.BYTE	$00		; Output enabled (CTRLOFG)	$45
	.WORD	$0014		; Initial lines counter(20)	$46-47
	.WORD	$0014		; Initial lines number (20)	$48-49
	.WORD	$0000		; Array load/save check sum	$4A-$4B
	.BYTE	$00		; Break not by NMI		$4C
	.BYTE	$00		; Break flag		$4D
	.BYTE	$00		; CURPOS	$4E
	.BYTE	$00		; LCRFLG Locate/Create Flag	$4F
	.BYTE	$00		; TYPE Data type flag	$50
	.BYTE	$00		; DATFLG literal statement flag	$51
	.BYTE	$00		; FORFLG "FOR" loop flag	$52
	.BYTE	$00		; Last byte entered		$53
	.BYTE	$00		; READ/INPUT flag		$54
	.WORD	-2		; Current LINE NUMBER	$55-$56
INITX	.BYTE	$00		; END OF INITIALISATION TABLE
;------------------------------------------------------------------------------
; BASIC ERROR CODE LIST
;------------------------------------------------------------------------------
ERRORS:	.BYTE	$01,"NEXT without FOR"
	.BYTE	$00,$02,"Syntax"
	.BYTE	$00,$03,"RETURN without GOSUB"
	.BYTE	$00,$04,"Out of DATA"
	.BYTE	$00,$05,"Illegal function call"
	.BYTE	$00,$06,"Overflow"
	.BYTE	$00,$07,"Out of Memory"
	.BYTE	$00,$08,"Undefined Line"
	.BYTE	$00,$09,"Bad Subscript"
	.BYTE	$00,$0A,"Re-DIM'd array"
	.BYTE	$00,$0B,"Division by zero"
	.BYTE	$00,$0C,"Illegal direct"
	.BYTE	$00,$0D,"Type Mismatch"
	.BYTE	$00,$0E,"Out of string space"
	.BYTE	$00,$0F,"String too long"
	.BYTE	$00,$10,"String too complex"
	.BYTE	$00,$11,"Can't CONTinue"
	.BYTE	$00,$12,"Undefined Function"
	.BYTE	$00,$13,"Missing Operand"
	.BYTE	$00,$14,"Stack Overflow"
	.BYTE	$00,$15,"Not valid HEX"
	.BYTE	$00
;------------------------------------------------------------------------------
; TEXT MESSAGES
;------------------------------------------------------------------------------
SIGNON:	.BYTE	$0C
	.BYTE	"Space-Time Productions MCB    "
	.BYTE	$A9,"1982"
	.BYTE	CR,LF
	.BYTE	"Microsoft BASIC V4.7 [Nascom] "
	.BYTE	$A9,"1978"
	.BYTE	CR,LF
	.BYTE	"'Boot to BASIC' V1.0 portover "
	.BYTE	$A9,"2011"
	.BYTE	CR,LF,0,0
	
SRAM:	.BYTE	" Bytes System Ram"
	.BYTE	CR,LF,0,0
BFREE:	.BYTE	" Bytes BASIC Available"
	.BYTE	CR,LF,0,0
...
TSAVE:	.BYTE	"Capture text file now.",CR,LF,0,0
TLOAD:	.BYTE	"Send Program file now.",CR,LF,0,0
CKSUMERR:	.BYTE	"Checksum Error in file.",CR,LF,0,0
;------------------------------------------------------------------------------
...
; INITIALIZE HARDWARE
;------------------------------------------------------------------------------
HWINIT:	XOR	A		; Start by clearing ram
	LD	HL,$3000
	LD	DE,$3001
	LD	BC,$0800
	LD	(HL),A
	...
	LD	SP,$3080		; Set the Stack Pointer low in ram
;------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------
PIT:	LD   E,$02	; PIT(CTC) # 2	
	LD   HL,$4E20	; 20,000 X 0.5 uSec =100 Hz if desired NMI/RTC
	CALL PIT1		; Write it to PIT
	LD   E,$01	; PIT(CTC) # 1	
	LD   HL,$01A1	; 417 x 0.5 uSec =4796 Hz / 16 = 299 (300 Baud)	
	CALL PIT1		; Write it to PIT
	LD   E,$00	; PIT(CTC) # 0	
	LD   HL,$001A	; 26 x 0.5 uSec = 76,923 Hz / 16 = 4807 (4800 Baud)
	CALL PIT1		; Write it to PIT
	JR   INITSIO	; Jump over the PIT routines, continue
;------------------------------------------------------------------------------
; This routine sets up the Programmable Interval Timer
;------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------
; Note here that we don't initialize the following:
; IC-20 p8279	Don't know how the user has this wired, 3D/4K or 8 column
; IC-14 p8255	Don't know how the user has this configured for his hdw
; IC-15 p8255	Ditto
; IC-16 p8255	Ditto
; IC-17 Am9511	Don't know if it exists, BASIC isn't using it anyway
; IC-19 Am9513	Don't know if it exists, BASIC isn't using it either
;------------------------------------------------------------------------------
; End of Hardware Initialization
;------------------------------------------------------------------------------
HDEND:	JP	CSTART		; Jump to BASIC Coldstart
;------------------------------------------------------------------------------
; Data bytes for the SIO
;------------------------------------------------------------------------------
SIODAT: 	.BYTE	$18		; SIO CONTROL, RST CH, SEL REGISTER 0
	.BYTE	$18		; WRITE DATA TO REGISTER 0
	.BYTE	$04		; SELECT REGISTER 4
	.BYTE	$44		; X 16 CLK, 1 STOP, No parity
...
;------------------------------------------------------------------------------		
; These are the default jump vectors for Z80 \INT and \NMI
;  They are copied to $3214 JP, $3215 L, $3216 H for INT
;                     $3217 JP, $3218 L, $3219 H for NMI
; The user can change these after bootup to alter the destination of INT, NMI
;  for own purposes. Beware of vectoring off to the weeds.
;------------------------------------------------------------------------------
VECTORS:	.ORG	$1FE0		; Soft configurable NMI and INT
INTSRC:	JP	WSTART		; BASIC Warmstart until user changes
NMISRC:	JP	WSTART		; Basic Warmstart until user changes
	NOP
	NOP
	NOP
	NOP
	...
	NOP
;------------------------------------------------------------------------------
; The following vectors are positioned at the top of BASIC's Rom so that 
;  users of USR(x) can easily locate DEINT and PASSA and ABPASS, regardless
;  of updates to BASIC.ASM which would shift their position inside code.
; DEINT takes the value x from USR(x) -32768 to 32767 from FPREG into regs DE
; ABPASS returns value from user's code 16-bit signed int and returns it to
;   Basic, so Z=USR(X), Z will contain result of user's program
; PASSA returns unsigned 8 bit integer 0 to 255 in reg A back to Basic.
;------------------------------------------------------------------------------
; Notes on using Machine Code with BASIC:
;------------------------------------------------------------------------------
; CLEAR 100,14235 causes basic to create a 100 byte string space, and to
;   set LSTRAM from $37FF to $379B (14235), leaving 100 bytes for your machine
;   code program which won't be overwritten by BASIC text.
; Be sure to set the location of your machine code program with VECTOR(x)
;   because the default USR vector will return a Function Call Error.
; Then Z=USR(X) is the format for passing variables to/from your program.
;   X is the signed 16-bit value you pass to your program, Z is the value ret'd.
;   Your program should first CALL DEINT to get the value into registers DE.
;   Then, if you are returning a signed 16-bit value, CALL this 
;   vector for JP ABPASS, then finish your program with a RET command.
;   If you are returning only an unsigned 8-bit value 0-255, call this
;   vector for JP PASSA,  then finish your program with a RET commmand.
;------------------------------------------------------------------------------		
	.ORG	$1FF0	; Standardized location of User Vectors
	JP	CSTART	; BASIC Cold Start (clears ram, resets all)
	JP	WSTART	; BASIC Warm Start (returns to OK prompt)
	
	JP	DEINT	; Pass Integer variable to user program in DE
	JP	ABPASS	; Pass 16-bit int from user back to BASIC
	JP	PASSA	; PASS INT 0-255 IN A	  (var back to bas)
;------------------------------------------------------------------------------
;------------------------------------------------------------------------------
